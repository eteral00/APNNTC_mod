...
#pragma unroll
    for (int tile_k = int(9*CIN/128/CHUNK_K)*CHUNK_K; tile_k < 9*CIN/128; tile_k++) {
      ...
      if (threadIdx.x < 252) *((int*)&shmem[SHMEM_i][0] + t) = *((int*)&X[GL_idx] + t);
...
#pragma unroll
    ...
    if (threadIdx.x<64) {
      ...
      int SHMEM_row = (threadIdx.x+20*64)/64;
      int SHMEM_col = threadIdx.x%64;
      int Output_row = SHMEM_row/7;
      int Output_col = SHMEM_row%7;
...
int main(int argc, char **argv) {
  ...
  /* validation results */
  validate_results(Output_h, C_ref, Height_no_pad, Width_no_pad, COUT);
#endif
