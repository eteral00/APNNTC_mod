...
#pragma unroll
    for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
      ...
      int *shmem_ptr;
      if (warpId < 4) {
        lane_ptr = (int*)W + (threadIdx.x/48)*W_bit_offset*4 + block_tile_i*K_GLOBAL/32 + (threadIdx.x%48)/4*K_GLOBAL/32 + (threadIdx.x%48)%4 + tile_k*4;
      ...
      *shmem_ptr = *lane_ptr;
      ...
      if (warpId < 4) {
        lane_ptr = (int*)W + ((128+threadIdx.x)/48)*W_bit_offset*4 + block_tile_i*K_GLOBAL/32 + ((128+threadIdx.x)%48)/4*K_GLOBAL/32 + ((128+threadIdx.x)%48)%4 + tile_k*4;
...
int main(int argc, char **argv) {
  ...
  int W_BIT = 5;
    ...
    int M_GLOBAL;
    if (M_GLOBAL_no_pad%12 != 0) {
      M_GLOBAL = (M_GLOBAL_no_pad/12+1)*12;
    } else {
  ...
  for (int N_GLOBAL=128; N_GLOBAL<=2048; N_GLOBAL += 128 ) {
    ...
    printf("V88, 64x64. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL_no_pad, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
