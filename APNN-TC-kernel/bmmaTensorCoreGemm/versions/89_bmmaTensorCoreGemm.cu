...
__global__ void apmm_w6a2(const int4 *W, const int4 *X, int *D, int M_GLOBAL, int N_GLOBAL, int K_GLOBAL, int wb, int xb) {
  // GEMM configuration.
  // printf("ckpt0\n");
...
#pragma unroll
    for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
      ...
      int *shmem_ptr;
      if (warpId < 4) {
        lane_ptr = (int*)W + (threadIdx.x/40)*W_bit_offset*4 + block_tile_i*K_GLOBAL/32 + (threadIdx.x%40)/4*K_GLOBAL/32 + (threadIdx.x%40)%4 + tile_k*4;
        // lane_ptr = (int*)W + (threadIdx.x/42)*W_bit_offset*4;
        shmem_ptr = (int*)shmem + (threadIdx.x/4)*4*(CHUNK_K+SKEW) + threadIdx.x%4;
      } else {
        lane_ptr = (int*)X + block_tile_j*K_GLOBAL/32 + (threadIdx.x-128)/4*K_GLOBAL/32 + threadIdx.x%4 + tile_k*4;
        shmem_ptr = (int*)shmem + 64*4*(CHUNK_K+SKEW) + (threadIdx.x-128)/4*4*(CHUNK_K+SKEW) + threadIdx.x%4;
      }
      *shmem_ptr = *lane_ptr;

      if (warpId < 4) {
        lane_ptr = (int*)W + ((128+threadIdx.x)/40)*W_bit_offset*4 + block_tile_i*K_GLOBAL/32 + ((128+threadIdx.x)%40)/4*K_GLOBAL/32 + ((128+threadIdx.x)%40)%4 + tile_k*4;
        shmem_ptr = (int*)shmem + ((128+threadIdx.x)/4)*4*(CHUNK_K+SKEW) + threadIdx.x%4;
        if (threadIdx.x < 112) *shmem_ptr = *lane_ptr;
      } else {
        lane_ptr = (int*)X + X_bit_offset*4 + block_tile_j*K_GLOBAL/32 + (threadIdx.x-128)/4*K_GLOBAL/32 + threadIdx.x%4 + tile_k*4;
        shmem_ptr = (int*)shmem + 96*4*(CHUNK_K+SKEW) + (threadIdx.x-128)/4*4*(CHUNK_K+SKEW) + threadIdx.x%4;
        *shmem_ptr = *lane_ptr;
      }
...
#pragma unroll
    ...
    *(D + block_tile_i*N_GLOBAL + block_tile_j + threadIdx.x/32*N_GLOBAL + threadIdx.x%32) = val;
    ...
    if (threadIdx.x < 64) {
      ...
      val = *(shmem_warp_stream_ptr) + 2*(*(shmem_warp_stream_ptr+32)) 
              ...
              + 16*(*(shmem_warp_stream_ptr+40*64)) + 32*(*(shmem_warp_stream_ptr+40*64+32))
              + 32*(*(shmem_warp_stream_ptr+50*64)) + 64*(*(shmem_warp_stream_ptr+50*64+32));
      *(D + block_tile_i*N_GLOBAL + block_tile_j + 8*N_GLOBAL + threadIdx.x/32*N_GLOBAL + threadIdx.x%32) = val;  
    }
...
int main(int argc, char **argv) {
  ...
  int W_BIT = 6;
    ...
    int M_GLOBAL;
    if (M_GLOBAL_no_pad%10 != 0) {
      M_GLOBAL = (M_GLOBAL_no_pad/10+1)*10;
    } else {
  ...
  for (int N_GLOBAL=128; N_GLOBAL<=1024; N_GLOBAL += 128 ) {
    ...
    printf("V89, 64x64. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL_no_pad, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
