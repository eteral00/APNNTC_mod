...
#pragma unroll
    for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
      ...
      int *shmem_ptr;
      if (warpId < 4) {
        lane_ptr = (int*)W + (threadIdx.x/84)*W_bit_offset*4 + block_tile_i*K_GLOBAL/32 + (threadIdx.x%84)/4*K_GLOBAL/32 + (threadIdx.x%84)%4 + tile_k*4;
      ...
      *shmem_ptr = *lane_ptr;
      ...
      if (warpId < 4) {
        lane_ptr = (int*)W + ((128+threadIdx.x)/84)*W_bit_offset*4 + block_tile_i*K_GLOBAL/32 + ((128+threadIdx.x)%84)/4*K_GLOBAL/32 + ((128+threadIdx.x)%84)%4 + tile_k*4;
        shmem_ptr = (int*)shmem + ((128+threadIdx.x)/4)*4*(CHUNK_K+SKEW) + threadIdx.x%4;
        if (threadIdx.x < 124) *shmem_ptr = *lane_ptr;
      } else {
...
int main(int argc, char **argv) {
  ...
  for (int N_GLOBAL=128; N_GLOBAL<=1024; N_GLOBAL += 128 ) {
    ...
    printf("V87, 64x64. M_GLOBAL: %d, N_GLOBAL: %d, K_GLOBAL: %d, X_BIT: %d, W_BIT: %d\n", M_GLOBAL_no_pad, N_GLOBAL, K_GLOBAL, X_BIT, W_BIT);
    printf("Time: %f ms\n", bmma_ms_avg);  
